...
#
#   Modified: MC JJA (c) 2016-17 by Janko J A
#
# --------------------------------------------------------
...
def alt_str_name_mc(name, src='unset', aieou=1):
	...
	# jja tmp block
	pattern = re.compile(ur'(.+)(\sst[\.]$)', re.UNICODE)
	try:
		name = ' '.join((pattern.search(name).group(1), u'sétány'))
	except:
		none = ''
	# jja block start
	name = name.replace(u'  ', u' ')
...
def mmqgis_geocode_street_layer(qgis, layername, csvname, streetnamefield, numberfield, zipfield, \
	...
	for index, field in enumerate(header):
		...
		if field == 'srch_name': # jja for debug srch_name
			srch_name_index_mc = index # jja for debug srch_name
		fields.append(QgsField(field[0:10].strip(), QVariant.String, "String", 254, 0))

	# jja: misspelling level set from QTForm
	aeiou_qual = int(float(aeiou_qual)) # jja
	aeiou = int( 1000 * ( 10 - aeiou_qual)) # jja
	zip_allowance = int(float(zip_allowance)) # jja
	nr_allowance = int(float(nr_allowance)) # jja
	...
	addresses = list(reader)
	address_count = len(addresses) ## jja

	# Convert attributes to unicode and street number to an integer
	rownum = 0
	while (rownum < address_count):
		try:
			addresses[rownum] = [unicode(field, "utf-8") for field in addresses[rownum]]
		except:
			return "CSV file must be in UTF-8 encoding"
		addresses[rownum] = addresses[rownum] + [0] + [0] # jja for quality+ debug
		try:
#			addresses[rownum][numberfield_index] = int(addresses[rownum][numberfield_index]) # org
			# allow float for /a - /g # jja
			addresses[rownum][numberfield_index] = float(addresses[rownum][numberfield_index]) # jja
			rownum = rownum + 1
		except:
...
#		feature_streetname = mmqgis_searchable_streetname(unicode(attributes[streetname_attribute])) # org
		ftr_alt_strname = alt_str_name_mc(unicode(attributes[streetname_attribute]), 'xref', aeiou_qual) # jja
		feature_streetname = ftr_alt_strname # jja for compatipility
		if not feature_streetname:
			continue
		# print "Searching street:", feature_streetname

		# Check each address against this feature
		feature_indx_mc += 1 # jja
		row_index = 0
		while (row_index < address_count):
			row = addresses[row_index]
			number = row[numberfield_index]

			alt_strname = alt_str_name_mc(unicode(row[streetnamefield_index]), 'addr', aeiou_qual) # jja
			# street = mmqgis_normalize_address(row[streetnamefield_index])[1]
#			street = mmqgis_searchable_streetname(unicode(row[streetnamefield_index])) # org
			street = alt_strname # jja for compatipility

			# print "   Searching address:", number, street
			if (not street) or (feature_streetname != street):
				row_index = row_index + 1
				continue
			row[srch_name_index_mc] = alt_strname
			try:
				diff_nr_test = diff_nr[row_index]
			except:
				diff_nr[row_index] = 99999
			try:
				zipp_nr_test = zipp_nr[row_index]
			except:
				zipp_nr[row_index] = 0

			gml_oid_number = int(attributes[gml_oid_attribute]) # jja
			debug_indx_int[feature_indx_mc,row_index] = 750 # jja street name matched (assuming that postal code fully matched)
			row[qual_index_mc] = aeiou + debug_indx_int[feature_indx_mc,row_index] # jja
			...
			left = ((leftfrom_number % 2) == (number % 2)) # jja outdented here
			...
			QgsMessageLog.logMessage(''.join(('zip_allowance: ',str(zip_allowance),';nr_allowance: ',str(nr_allowance)))) # jja
			if zip_allowance == 2:
				zip_diff = 9
				QgsMessageLog.logMessage(''.join(('zip_diff: ',str(zip_diff)))) # jja
			else:
				zip_diff = 0
				QgsMessageLog.logMessage(''.join(('zip_diff: ',str(zip_diff)))) # jja
			try:
...
#				QgsMessageLog.logMessage('zipcode as was')
			...
			if not (zipcode == rightzipcode) and not (zipcode == leftzipcode):
				debug_indx_int[feature_indx_mc,row_index] = 550 # jja postal code not matched exactly
				row[qual_index_mc] = aeiou + debug_indx_int[feature_indx_mc,row_index]
			if not leftzip and not rightzip:
				debug_indx_int[feature_indx_mc,row_index] = 50 # jja postal code not matched
				row[qual_index_mc] = aeiou + debug_indx_int[feature_indx_mc,row_index]
			QgsMessageLog.logMessage(''.join(('qual_index_mc: ',str(row[qual_index_mc])))) # jja
			QgsMessageLog.logMessage(''.join(('zipcode: ',str(zipcode),';leftzipcode: ',str(leftzipcode),';rightzipcode: ',str(rightzipcode)))) # jja
			QgsMessageLog.logMessage(''.join(('leftzip: ',str(leftzip),';rightzip: ',str(rightzip)))) # jja
			...
			if ((leftto_number >= leftfrom_number) \
			   ...
			   ((rightto_number < rightfrom_number) \
				...
				# print "      Match!"
				debug_indx_int[feature_indx_mc,row_index] += 3 # jja house nr matched
				row[qual_index_mc] = aeiou + debug_indx_int[feature_indx_mc,row_index]
...
#				QMessageBox.information(None, "DEBUG geometry.length:", str(geometry.length())) # jja
				...
				diff_nr[row_index] = 0
				QgsMessageLog.logMessage(''.join(('debug_indx_int[feature_indx_mc,row_index]: ',str(debug_indx_int[feature_indx_mc,row_index]))))
				del debug_indx_int[feature_indx_mc,row_index] # jja match quality measure

#			else:
#				row_index = row_index + 1 # org -> jja down

			try:
#				QgsMessageLog.logMessage(''.join(('feature_indx_mc: ',str(feature_indx_mc),';row_index: ',str(row_index))))
				if nr_allowance  == 2 and debug_indx_int[feature_indx_mc,row_index] > 50:
					# collect street name matches
					if from_number_mc < 0:
						debug_indx_int[feature_indx_mc,row_index] -= 40
						row[qual_index_mc] = aeiou + debug_indx_int[feature_indx_mc,row_index]
					diff_nr_test = min(abs(from_number_mc - number), abs(to_number_mc - number))
					if diff_nr_test < diff_nr[row_index]:
						diff_nr[row_index] = diff_nr_test # jja
						approx_rows.append(row)
						geom_mc = feature.geometry()
						length_mc = geom_mc.length() # jja
						(x, y, angle) = lin_int_mc(geom_mc, length_mc/2)
						meta_rows.append({'rid':row_index,'diff':diff_nr_test,'oid':gml_oid_number,'x':x,'y':y})
			except:
				do = None
			row_index = row_index + 1 # down here
...
#			QgsMessageLog.logMessage(str(arow[qual_index_mc]))
#			QgsMessageLog.logMessage(str(arow[numberfield_index]))
#			QgsMessageLog.logMessage(str(meta_rows[i]['diff']))
#			QgsMessageLog.logMessage(str(meta_rows[i]['oid']))
...
#			QgsMessageLog.logMessage(str(meta_rows[i]['diff']))
	...
	mmqgis_completion_message(qgis, unicode(matched_count) + " of " + unicode(address_count) \
		+ " addresses geocoded from " + unicode(feature_count) + " street records") ## jja
